...
def prepare_plot_data(df, vertex_count, k_value):
    ...
    filtered_df = df[(df['vertex_count'] == vertex_count) & (df['k'] == k_value)]
...
def main():
    ...
    df_BA = pd.read_csv('comparisons/comparison_of_centralities_vs_gradient_descend_BA.csv')
    vertex_counts = df_BA['vertex_count'].unique()  # Automatically handle different vertex counts

    for vertex_count in vertex_counts:
        k_values = df_BA[df_BA['vertex_count'] == vertex_count]['k'].unique()
        for k_value in k_values:
            plot_df = prepare_plot_data(df_BA, vertex_count, k_value)
            plot_violin(plot_df, f'BA Graphs with {vertex_count} Vertices, k={k_value}')
