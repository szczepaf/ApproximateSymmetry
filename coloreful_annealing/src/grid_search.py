...
import sa_eigenvector_centrality_two_step as sa_improved # import the given algorithm that we want to test
...
def run_simulation_on_parameters(graph_tuples, steps, division_constant, probability_constant, filename, alg_type):
    for graph_tuple in graph_tuples:
        ...
        #perm, S = sa_improved.annealing(nx.to_numpy_array(graph), steps=steps , fp = 5, probability_constant=probability_constant, division_constant=division_constant)
        perm, S = nsa.annealing(nx.to_numpy_array(graph), steps=steps , fp = float(inf))
...
def main():
    ...
    algorithm_type = "nsa_checking_fps"
    steps = 20000
    ...
    #remaining_twins = [[1, 0.1], [1, 1], [10, 0.001], [10, 0.01], [10, 0.2], [10, 0.1], [10, 1]]
    
    graph_tuples = load_graphs_from_csv("graphs_for_grid_search_26_03.csv")   # These are the used graphs  
    file_name = f"symmetry_results_{date}_{algorithm_type}_checking_fixed_points.csv"
