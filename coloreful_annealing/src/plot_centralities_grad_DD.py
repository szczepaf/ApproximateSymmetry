...
def prepare_plot_data(df, vertex_count, k_value):
    ...
    filtered_df = df[(df['vertex_count'] == vertex_count) & (df['probability'] == k_value)]
    ...
    for algorithm in algorithms:
        ...
        for energy in filtered_df[energy_column]:
            plot_data['probability'].append(k_value)
...
def main():
    ...
    df_BA = pd.read_csv('comparisons/comparison_of_centralities_vs_gradient_descend_DuplicationDivergence.csv')
    vertex_counts = df_BA['vertex_count'].unique()  # Automatically handle different vertex counts

    for vertex_count in vertex_counts:
        k_values = df_BA[df_BA['vertex_count'] == vertex_count]['probability'].unique()
        for k_value in k_values:
            plot_df = prepare_plot_data(df_BA, vertex_count, k_value)
            plot_violin(plot_df, f'DD Graphs with {vertex_count} Vertices, probability={k_value}')
