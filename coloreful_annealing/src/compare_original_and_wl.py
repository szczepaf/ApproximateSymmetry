...
def run_simulation_on_grid(graphs, steps, file_format):
    ...
    file_name = f"{file_format}_{graph_type}.csv"
    with open(file_name, "w") as f:
        f.write("type,vertex_count,dimension,og_fp,og_energy,wl_fp,wl_energy\n")
    ...
    for graph_tuple in graphs:
        ...
        og_perm, og_S = sa.annealing(adj_matrix, steps=steps)
        wl_perm, wl_S = wl.annealing(adj_matrix,probability_constant=0.9, iterations=1)
        ...
        # store the results in a csv file
        with open(file_name, "a") as f:
            f.write(f"{graph_type},{vertex_count},{dimension},{og_fp},{og_S},{wl_fp},{wl_S}\n")
...
def main():
    ...
    file_format = "comparisons/comparison_of_og_and_wl"
    ## TODO: change and run the rest
    steps = 30000
    #print("BA")
