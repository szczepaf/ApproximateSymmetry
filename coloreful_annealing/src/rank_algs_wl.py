...
data = pd.read_csv('analyzed_grid_search_results/wl.csv')
...
for graph_type, vertex_count in categories:
    ...
    for rank, row in enumerate(category_df.itertuples(), 1):
        params = (row.iteration, row.probability)
        # Update scores
        if params in scores:
            scores[params] += rank
...
best_params_df.to_csv(f"analyzed_grid_search_results/ranked_params_wl.csv", index=False)
