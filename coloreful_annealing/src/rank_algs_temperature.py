...
data = pd.read_csv('analyzed_grid_search_results/temperatures.csv')
...
for graph_type, vertex_count in categories:
    ...
    for rank, row in enumerate(category_df.itertuples(), 1):
        params = (row.temp_max, row.temp_min)
        # Update scores
        if params in scores:
            scores[params] += rank
...
best_params_df.to_csv(f"analyzed_grid_search_results/ranked_temperature.csv", index=False)
