...
def prepare_plot_data(df, vertex_count, k):
    ...
    filtered_df = df[(df['vertex_count'] == vertex_count) & (df['dimension'] == k)]
    algorithms = ['betweenness', 'og']
    
    for algorithm in algorithms:
        energy_column = f'{algorithm}_energy'
        for energy in filtered_df[energy_column]:
            plot_data['dimension'].append(f'{k}')
...
def main():
    df_BA = pd.read_csv('comparisons/comparisons_of_betweenness_vs_og_on_large_grid.csv')
    ...
    for comparison in comparisons:
        ...
        for vertex_count in vertex_counts:
            for k in k_values:
                df_filtered = df_BA[(df_BA['vertex_count'] == vertex_count) & (df_BA['dimension'] == k)]
                ...
                cohens_d_df.loc[vertex_count, f'{k}'] = cohens_d
